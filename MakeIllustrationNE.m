...
% on line 67.
function[sum_pay]= MakeIllustrationNE();
...
S1=[0.065-0.028 0.120]; red=[0.8 0 0];  msS=8; lwS=2;
...
Input_file = 'Inputs/InputDataXsetSize2_24_2_SigmoidThreshold_OneEighth_NewDefOfOverallCoop.mat';
%Input_file = 'Inputs/InputData_exact_Fig2_1MnGen.mat';
%Input_file = 'Inputs/InputDataXsetSize3_29_SigmoidThreshold_3.mat';
%Input_file = 'Inputs/InputDataXsetSize3_22_Prime_SigmoidThreshold_1_NewDefOfOverallCoop_TwoTimesMoreGen.mat';
...
%     end
    ...
    sz_tmp = size(E1);
    nPlayer = sz_tmp(2);
    X_OneMinus = [0 1 1;1 0 1;1 1 0];
    X_Zero = [0 0 0];
    X_One = [1 1 1];
    ProbabilityOfNextRound = 0.8; % delta in Proposition 1 page 9 suppl info
    U_Zero = PayoffCalculator(E1, X_Zero, rvec, candidates(nr + 1));
    U_One = PayoffCalculator(E1, X_One, rvec, candidates(nr + 1));
    U_OneMinus = zeros(nPlayer, nPlayer);
    for i = 1:nPlayer
        U_OneMinus(i,:) = PayoffCalculator(E1, X_OneMinus(i,:), rvec, candidates(nr + 1));
    end
    
    rejected_rules = 0;
    for i = 1:nPlayer
        if ProbabilityOfNextRound * (U_OneMinus(i, i) - U_Zero(i)) < U_OneMinus(i, i) - U_One(i)
            rejected_rules = rejected_rules + 1;
        end
    end
